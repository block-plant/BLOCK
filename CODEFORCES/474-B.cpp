...
void solve() {
    ...
    while(m--)
    {
        ...
        ll idx = upper_bound(all(pref) , q) - pref.begin();
        if(idx > 0)
        {
            if(pref[idx-1] == q)
            {
...
// ab count ess problem me jese horha usse prefix sum ka hint mil ja rha 
// prefix sum pr search krlo ki ek specific count kahan h , upper_bound se check kro
// aisa ho skta h ki 12 ke liye check kro and 12 prefix sum array me ho ,
// toh upper_bound next index dega , esse liye past index check krlo ki kahin vo query ke barabar toh nahi h
// agar nahi h toh upperbound hi ans h , otherwise -1 kro bcoz peeche vala hoga ans (index)