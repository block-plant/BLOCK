...
vector<int> get_char_freq(const string& s) { vector<int> freq(26, 0); for (char c : s) if (c >= 'a' && c <= 'z') freq[c - 'a']++; return freq; }
...
void solve() {
    ...
    f(i , 0 , n)
    {
        ...
        {
            ll req = ta / 2 ;
            if(req >= 0 && req <= 1000000)
            {
                if(req == a[i] && freq[req] > 1) 
                {
                    ans.push_back(i+1);
                }
                else if(req != a[i] && freq[req] > 0)
                {
                    ans.push_back(i+1);
                }
            }
