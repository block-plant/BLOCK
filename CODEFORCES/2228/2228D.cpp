...
void solve() {
    ...
    f(i , 0 , len - 1) {
        other = other * 10 + d[n-1] ;
    }
    if(len > 1) ans = min(ans , (unsigned long long)(a - other));

    unsigned long long lng = d[d[0] == 0];
    f(i , 0 , len) {
        lng = lng * 10 + d[0];
    }
    ans = min(ans , (unsigned long long)(lng - a));

    auto dfs = [&](auto& self , ll val , ll cur , ll idx)->void {
        if(idx == len) {
            ans = min(ans, (unsigned long long)abs(a - val));
            return;
        }
        ...
        for(ll x : d) {
            ...
            if(cur == -1) {
                self(self , val * 10 + d[n-1] , -1 , idx+1);
                break;
            }
