...
void solve() {
    ...
    ll first = mul(n,(power(m,n-1) % MOD)) ; 
    ll sum_M = mul(mul(m % MOD, (m + 1) % MOD), modInverse(2));
    ll newfirst = mul(first, sum_M);

    ll ans = 0 ; 
    f(k , 1 , m+1)
    {
        ll second = power(k , n) % MOD ; // it can be power of k-n+1 and n
        ans = add(ans, second);
    }
    cout << sub(newfirst, ans) << endl;
}
