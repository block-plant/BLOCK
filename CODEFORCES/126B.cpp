...
void solve() {
    ...
    build_hash(s, pref, suff);

    auto check = [&](ll mid)->bool {
        if(mid == 0) return true;
        auto target = get_hash(0 , mid - 1 , pref , suff);
        f(i , 1 , n - mid) {
            if(get_hash(i , i + mid - 1 , pref , suff) == target) {
                return true; 
            }
        }
        return false;
    ...
    ll len = 0 ;
    fr(i , ans , 1) {
        auto pre = get_hash(0, i - 1, pref, suff);
        auto suf = get_hash(n - i , n - 1 , pref , suff);
        if(pre == suf) {
            len = i ; 
            break;
        }
